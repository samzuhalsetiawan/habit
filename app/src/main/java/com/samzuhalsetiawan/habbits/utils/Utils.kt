...
inline fun <reified VM: ViewModel> provideViewModel(
    owner: ViewModelStoreOwner = (LocalContext.current.getActivity() as MainActivity)
): VM {
    return viewModel<VM>(viewModelStoreOwner = owner)
}
...
val NavBackStackEntry?.currentRoute : String?
    get() = this?.destination?.route?.substringBefore("?")?.substringBefore("/")?.substringAfterLast(".")
