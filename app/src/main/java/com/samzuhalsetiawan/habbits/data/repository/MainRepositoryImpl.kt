...
class MainRepositoryImpl(
   ...
   override suspend fun saveAnsweredQuestion(answeredQuestion: AnsweredQuestion): MainRepositoryResult<Unit> {
      return handleRequestAsync {
         answeredQuestion.let {
            habitDB.startingQuestionDao.insertQuestionResult(
               StartingQuestion(
                  it.question,
                  it.answer,
                  it.score
   ...
   private suspend fun <T> handleRequestAsync(
      onError: (suspend (throwable: Throwable) -> Unit)? = null,
      action: suspend () -> T
   ): MainRepositoryResult<T> {
      return try {
